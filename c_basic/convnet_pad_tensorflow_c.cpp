...
void convnet_padhamsu_tensorflow(int* image, int* kernel, int stride, int* padmap);

int image1[] = { 3, 2, 4, 5, 2, 3, 1, 0, 4, 2, 1, 4, 3, 4, 7, 2 };
int kernel1[] = { 1, 1, 0, 0, 1, 0, 1, 0, 1 };
int stride1[] = { 2,2 };

int image_length = sqrt(sizeof(image1) / sizeof(int));		// 4 
int kernel_length = sqrt(sizeof(kernel1) / sizeof(int));	// 3
int stride = stride1[0];									// 2
int pad_length = stride * (image_length - 1) - image_length + kernel_length;	//

int main()
{
	int* image = (int*)malloc(sizeof(image1) / sizeof(int));
	int* kernel = (int*)malloc(sizeof(kernel1) / sizeof(int));
	int* padmap = (int*)malloc((image_length + pad_length)*(image_length + pad_length));
...
*/
	...
	convnet_padhamsu_tensorflow(image, kernel, stride, padmap);
